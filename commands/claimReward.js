    ...
    for (let { deposit, user } of depositors) {
        try {
            ...
            if (depositData === null) {
                ...
                console.log(
                    `the depositors amount is ${depositSize} and the total deposit is ${depositAmount}`
                );
                if (depositSize == 0) {
                    continue;
                }
                const depositorCoin = coin
                    .mul(depositSize)
                    .div(depositAmount)
                    .toString();
                const depositorToken = token
                    .mul(depositSize)
                    .div(depositAmount)
                    .toString();
                const depositorGovernance = governance
                    .mul(depositSize)
                    .div(depositAmount)
                    .toString();
                ...
                instructions.push(
                    await createAddRewardIx({
                        deposit,
                        coin: depositorCoin,
                        token: depositorToken,
                        governance: depositorGovernance,
                    })
                );
            }
        } catch (e) {
